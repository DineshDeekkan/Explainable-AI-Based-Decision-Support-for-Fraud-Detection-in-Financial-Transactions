...
while True:
    ...
    for message in consumer.poll(timeout_ms=500).values():
    ...
    with placeholder.container():
        ...
        if fraud_rate > 30:
            ...
            st.warning("⚠️ Moderate Fraud Activity")
        ...
        # 📈 TREND CHART
        # =============================
        st.markdown("### 📈 Fraud Trend")

        if not df.empty:
            trend = df["predicted_label"].apply(lambda x: 1 if x == "FRAUD" else 0)
            st.line_chart(trend.tail(50))
        ...
        with left:
            ...
            if metrics_data:
                ...
                st.info(
                    f"TP:{metrics_data.get('TP',0)} | "
                    f"TN:{metrics_data.get('TN',0)} | "
                    f"FP:{metrics_data.get('FP',0)} | "
                    f"FN:{metrics_data.get('FN',0)}"
                )
            else:
